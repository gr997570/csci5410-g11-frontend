...
export class ChatComponent implements OnInit {
  ...
  getUsers(){
    ...
    this.http.get("http://localhost:8080/chat/"+user.email+"/"+user.boxnumber).subscribe((result:any)=>{
      if(result.status){
  ...
  getMessages(){
    ...
    this.http.post("http://localhost:8080/", user).subscribe((result:any)=>{
      if(result.status){
  ...
  onSubmit(){
    ...
    this.http.post("http://localhost:8080/chats/", message).subscribe((result:any)=>{
      if(result.status){
        console.log(result);
      }
    });
  ...
  upload(): void {
    ...
    if (this.selectedFiles) {
      ...
      if (file) {
        ...
        this.http.post("http://localhost:8080/upload", formData).subscribe((result:any)=>{
          if(!result.status){
